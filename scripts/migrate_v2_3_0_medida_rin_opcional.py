"""
Migration v2.3.0 — Medida de llanta con rin opcional.

Permite registrar dimensiones sin rin (selector "Rin / -"), p.ej. llantas
convencionales como 10.00-20 que no usan la medida de rin en el catálogo.

Cambio de esquema:
    medidas_llanta.rin  INTEGER NOT NULL  →  FLOAT (nullable)

SQLite no soporta "ALTER COLUMN DROP NOT NULL": la tabla se RECREA con el
mismo esquema (rin FLOAT nullable), se copian los datos (ids preservados), se
...
Uso:
    python scripts/migrate_v2_3_0_medida_rin_opcional.py --dry-run
    python scripts/migrate_v2_3_0_medida_rin_opcional.py --apply
...
# ── Schema nuevo (rin FLOAT nullable) ────────────────────────────────────
...
def contar_medidas(cur: sqlite3.Cursor) -> tuple[int, int]:
    """Devuelve (total_filas, filas_con_rin_null)."""
    cur.execute("SELECT COUNT(*) FROM medidas_llanta")
    total = cur.fetchone()[0]
    cur.execute("SELECT COUNT(*) FROM medidas_llanta WHERE rin IS NULL")
    nulos = cur.fetchone()[0]
    return total, nulos


def aplicar_migracion(cur: sqlite3.Cursor) -> None:
    """Recrea medidas_llanta con rin nullable, preservando datos e ids."""
...
def verificar(cur: sqlite3.Cursor) -> bool:
    ...
    if "rin FLOAT" not in sql or "rin NOT NULL" in sql:
        print("  [WARN]   rin no quedó FLOAT nullable")
        ok = False

    if ok:
        print("  [OK]  Verificación completa: BD íntegra, rin ahora nullable")
    return ok
...
def run(dry_run: bool) -> int:
    ...
    if dry_run:
        ...
        try:
            ...
            print(f"\n[ANTES]  medidas_llanta: {antes_total} filas ({antes_nulos} con rin NULL)")

            aplicar_migracion(cur)
            conn.commit()

            despues_total, despues_nulos = contar_medidas(cur)
            print(f"[DESPUÉS] medidas_llanta: {despues_total} filas ({despues_nulos} con rin NULL)")
            ...
            # Comprobar que ahora se puede insertar rin NULL
            cur.execute("INSERT INTO medidas_llanta (id, ancho, perfil, rin) VALUES (999999, 1, NULL, NULL)")
            cur.execute("DELETE FROM medidas_llanta WHERE id = 999999")
            print("  [OK]  Insert con rin NULL permitido")
    ...
    backup_path = BACKUP_DIR / f"delca_pre_v2_3_0_{fecha}.db"
    ...
    try:
        ...
        ok = verificar(cur)
        if ok:
            print("\n[OK]  Migración v2.3.0 aplicada correctamente.")
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Migración v2.3.0 — medida de llanta con rin opcional"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
